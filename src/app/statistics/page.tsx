    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-4">
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-500">Доходи</p>
          <p className="text-xl font-bold text-emerald-400 mt-1">+{formatCurrency(totalIncome)}</p>
        </div>
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-500">Витрати</p>
          <p className="text-xl font-bold text-red-400 mt-1">-{formatCurrency(totalExpenses)}</p>
        </div>
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-500">Різниця</p>
          <p className={cn(
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-500">Сер. витрата / день</p>
          <p className="text-xl font-bold text-red-400 mt-1">
            {formatCurrency(avgExpensePerDay)}
          </p>
          <p className="text-[10px] text-zinc-600 mt-0.5">{daysInMonth} днів у місяці</p>
        </div>
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-500">Сер. дохід / день</p>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
        <h2 className="text-base font-semibold text-white mb-4">Тренд за 6 місяців</h2>
        <div className="h-56">
      ...
      {/* Expenses & Income by Category - side by side */}
      <div className="grid grid-cols-2 gap-4">
        {/* Expense Breakdown Pie */}
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
          <h2 className="text-base font-semibold text-white mb-4">Витрати по категоріях</h2>
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
          <h2 className="text-base font-semibold text-white mb-4">Доходи по категоріях</h2>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
        <h2 className="text-base font-semibold text-white mb-4">Доходи та витрати по днях</h2>
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
          <h2 className="text-base font-semibold text-white mb-4">Найбільші витрати</h2>
              ...
              {topExpenses.map((t, i) => {
                ...
                return (
                  <div key={t.id} className="flex items-center gap-3 rounded-xl bg-zinc-800/30 px-3 py-2.5">
                    ...
                    <p className="text-sm font-semibold text-red-400">-{formatCurrency(t.amount)}</p>
                  </div>
                );
              })}
            </div>
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
          <h2 className="text-base font-semibold text-white mb-4">Найбільші доходи</h2>
          {topIncomes.length > 0 ? (
            <div className="space-y-2">
              {topIncomes.map((t, i) => {
                const cat = state.categories.find(c => c.id === t.categoryId);
                return (
                  <div key={t.id} className="flex items-center gap-3 rounded-xl bg-zinc-800/30 px-3 py-2.5">
                    ...
                    <p className="text-sm font-semibold text-emerald-400">+{formatCurrency(t.amount)}</p>
                  </div>
                );
              })}
            </div>
